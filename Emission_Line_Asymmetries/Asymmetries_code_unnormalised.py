...
# This code script is or inputing only unnormalised spectra requiring a fitted
# continuum.
################################################################################
################################################################################
...
# for grid_point in range(50, 200, 1):
#     plt.plot(wavelengths, grid[grid_point], label='spectra')
#     plt.plot(wavelengths, sk_con_data[grid_point], label='continuum_predict')
#     plt.plot(wavelengths, sk_slopes[grid_point]*wavelengths+sk_intercepts[grid_point], label='calculated', linestyle='--')
#     plt.legend()
#     plt.show()
# %% 4
################################################################################
...
def fit_data(wavelengths, grid, sk_continuum):
    ...
    for run in range(len(grid)):
        # assuming sk_continuum[run] is a single value
        spectrum = grid[run, :] #/ sk_continuum[run]
        ...
        gaussian_only = spectrum - (initial_m * (wavelengths) + initial_c) # removing continuum
        ...
        # spectrum = spectrum[(wavelengths > H_alpha - int(blue_peak_mask[0])) & (wavelengths < H_alpha + int(red_peak_mask[0]))]
        # #sk_cont_run = sk_continuum[run][(wavelengths > H_alpha - int(blue_peak_mask[0])) & (wavelengths < H_alpha + int(red_peak_mask[0]))]
        # wavelengths_cut = wavelengths[(wavelengths > H_alpha - int(blue_peak_mask[0])) & (wavelengths < H_alpha + int(red_peak_mask[0]))]
        fix_continnum_gauss = partial(gaussian_with_continuum,
                                      m=sk_slopes[int(run)],
                                      b=sk_intercepts[int(run)],
                                      run=run
                                      ) # fixing variables
        parameters, pcov, infodict, _, _= curve_fit(fix_continnum_gauss, 
                                           wavelengths, 
                                           spectrum, 
                                           p0=initials, 
                                           maxfev=1_000_000,
        ...
        parameters = np.append(parameters, [sk_slopes[int(run)], sk_intercepts[int(run)], run]) # add run number to parameters
    ...
    parameters_all = parameters_all.reshape(len(grid), len(initials)+3)
...
def fit_procedure(wavelengths, grid, sk_continuum, sk_m, sk_c):
    ...
    fit_parameters, initials_all, pcov_all, infodict_all= fit_data(wavelengths, grid, sk_continuum) # optimal parameters for a spectrum
    # replace all the slopes and intercepts with the sk values
    fit_parameters[:, 3] = sk_m
    fit_parameters[:, 4] = sk_c
    ...
    fit_con = np.array([])
    for spectrum in range(len(grid)):
        fit_con = np.append(fit_con, fit_parameters[spectrum][3]*(wavelengths) + fit_parameters[spectrum][4])
...
# def fit_procedure(wavelengths, grid, sk_continuum):
#     """This function is to simplify the procedure of fitting the data. We take
#     the PYTHON grid and inital """
#     # Fitting the grid's emission peak AND continuum
#     fit_parameters, initials_all, pcov_all, infodict_all= fit_data(wavelengths, grid, sk_continuum) # optimal parameters for a spectrum
...
fitted_grid, fit_con, fit_parameters, initials_all, pcov_all, infodict_all = fit_procedure(wavelengths, grid, sk_con_data, sk_slopes, sk_intercepts) #TODO  this causes the covariance issue
...
for sample in tqdm(range(samples)):
    ...
    resampled_fitted_grid, resampled_fit_con, resampled_fit_parameters, resampled_initials_all, _, _ = fit_procedure(wavelengths, resampled_grid, sk_con_data, sk_slopes, sk_intercepts)
    blue = equivalent_width_excess(wavelengths, resampled_grid, resampled_fitted_grid, resampled_fit_con, shift='blue', peak_mask=blue_peak_mask)
    red = equivalent_width_excess(wavelengths, resampled_grid, resampled_fitted_grid, resampled_fit_con, shift='red', peak_mask=red_peak_mask)
...
def fwhm_calculations(wavelengths, data, continuum_fit):
    ...
    #index of the max flux value
    max_index = np.array([np.argmax(data[i]) for i in range(len(data))])
    
    # Searching for the blue FWHM bound
    blue_fwhms = np.array([])
    for i in range(len(data)): # iterate over each spectrum
        for j in range(len(data[i])): # iterate over each wavelength
            if data[i][j] >= max_flux[i]/2:
                blue_bound = wavelengths[j]
                if blue_bound > wavelengths[max_index[i]]: # don't search the wrong side of H_alpha
                    blue_bound = wavelengths[max_index[i]]
                break
    ...
    for i in range(len(data)):
        for j in range(len(data[i])):
            if data[i][::-1][j] >= max_flux[i]/2:
                ...
                if red_bound < wavelengths[max_index[i]]: # don't search the wrong side of H_alpha
                    red_bound = wavelengths[max_index[i]] # was H_alpha
                break
...
# Calculating the equivalent width excess data points for the PYTHON Grid
fitted_grid, fit_con, fit_parameters, initials_all, _, _ = fit_procedure(wavelengths, grid, sk_con_data, sk_slopes, sk_intercepts) #TODO  this causes the covariance issue
fwhm_bounds = fwhm_calculations(wavelengths, grid, fit_con)
...
for sample in tqdm(range(samples)):
    ...
    fwhm_bounds_resample = fwhm_calculations(wavelengths, resampled_grid, resampled_fit_con)
    fwhm = fwhm_bounds_resample[:,1] - fwhm_bounds_resample[:,0]
    fwhm_resample_all = np.append(fwhm_resample_all, fwhm)
...
#resampled grid shape is 150 * (run_number x number of wavelengths)
fwhm_resample_all = fwhm_resample_all.reshape(samples, len(grid))
fwhm_error = np.std(fwhm_resample_all, axis=0)
...
if i_want_to_view_the_fits:
    def slider_update(val):
        ...
        #ax.axvline(x=H_alpha +red_peak_mask[0], color='black', linestyle='--', alpha=0.5)
        #ax.axvline(x=H_alpha -blue_peak_mask[0], color='black', linestyle='--', alpha=0.5)
...
if grid_mixed and not script_ran_once:
    ...
    stored_results = {'wavelength_grid': wavelength_grid.tolist(),
                        ...
                        # 'fwhm_bounds': fwhm_bounds.tolist(), # comment out below if FWHM
                        # 'fwhm_error': fwhm_error.tolist(),
...
target = plt.scatter(final_results['red_ew_excess'][ranges],
                     final_results['blue_ew_excess'][ranges],
                     #c='black',
                     c=final_results['run_number'][ranges],
                     s=15,
                     label='Grid Data',
                     #alpha=0.2\
                     cmap='rainbow',
...
#plt.scatter(mv_lyr[:,0], mv_lyr[:,1], color='blue', s=10, marker='o', label = 'mv_lyr')
#plt.scatter(v425_cas[:,0], v425_cas[:,1], color='blue', s=10, marker='o', label = 'v425_cas')
...
def double_peak_detection(fluxes, continuum) -> list:
    ...
    fluxes -= continuum
    peak = signal.find_peaks(fluxes, 
                             height=0.3*np.max(fluxes), 
                             distance=10, 
                             prominence=0.1*np.max(fluxes))
    number = len(peak[0])
...
# removing emission lines with more than 3 peaks, 
# highlighting double vs single peaks
cut_runs_3 = []
peak_colour_map = []
test_runs = np.arange(0,729,1)
...
print(f"Removing {len(cut_runs_3)} triple+ peaks")
...
for i in range(len(removed_runs)):
    if final_results['red_ew_excess_error'][i] > 0.5 or final_results['blue_ew_excess_error'][i] > 0.5:
        ...
        cut_runs_2.append(i)
print(f"Removing {len(cut_runs_2)} high error runs")
...
cut_runs = [int(x) for x in cut_runs] #list of ints
...
# save ew
np.save(f'ew_data_all_inc{inclination_column}.npy', ew)
...
################################################################################
H_alpha = 6562.819 # central h_alpha line in angstroms
...
kms_to_angstrom(1200) #55Å(2500) 88Å(4000)

angstrom_to_kms(6562.819 + 55)

#print(H_alpha - kms_to_angstrom(1000)[0])
